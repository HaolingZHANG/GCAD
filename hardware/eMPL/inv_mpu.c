...
int8_t (*i2c_read)(uint8_t addr, uint8_t reg, uint8_t len, uint8_t * data);
...
#define AKM_CONTINUOUS2_MEASUREMENT  (0x06 | SUPPORTS_AK89xx_HIGH_SENS)
...
#ifdef AK89xx_SECONDARY
static int setup_compass(u8 sensorID);
#define MAX_COMPASS_SAMPLE_RATE (100)
short g_mag_sens_adj[7][3]={
		{304,304,293},
		{0,0,0},
		{0,0,0},
		{0,0,0},
...
void needChangeSlaveAddr(unsigned char sAddr)
{
     st.hw->addr = sAddr;
}
/**
 ...
 *  @return     0 if successful.
 */
int mpu_init(u8 slaveaddr,u8 sensorID)
{
    unsigned char data[6] = {0};

   st.hw->addr = slaveaddr;
   i2c_read(st.hw->addr, st.reg->who_am_i, 1, &(data[0]));
   if(data[0] != 0x71)
   {
	return -1;
   }
...
/*   for (akm_addr = 0x0c; akm_addr <= 0x0f; akm_addr++) {
        ...
        {
		printf("find compass,slave addr 0x%02x\r\n",akm_addr);
		break;
	}
    }
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
    g_mag_sens_adj[sensorID][0] = (long)data[0] + 128;
    g_mag_sens_adj[sensorID][1] = (long)data[1] + 128;
    g_mag_sens_adj[sensorID][2] = (long)data[2] + 128;

    data[0] = AKM_POWER_DOWN;
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    printf("mag sen adj:%d,%d,%d\r\n", st.chip_cfg.mag_sens_adj[0], st.chip_cfg.mag_sens_adj[1], st.chip_cfg.mag_sens_adj[2]);
*/		
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(u8 sensorID, short *data, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
...
//    printf("%04x, %04x %04x\r\n" , data[0],data[1],data[2]);
    data[0] = ((long)data[0] * g_mag_sens_adj[sensorID][0]) >> 8;
    data[1] = ((long)data[1] * g_mag_sens_adj[sensorID][1]) >> 8;
    data[2] = ((long)data[2] * g_mag_sens_adj[sensorID][2]) >> 8;
